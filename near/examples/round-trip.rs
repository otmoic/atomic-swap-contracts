...
mod test {
    ...
    async fn round_trip() -> Result<()> {
        ...
        let sender_balance = sender.view_account().await?.balance;
        ...
        let pending_event = check(&contract, &receiver, transfer_id).await?;
        ...
        let new_receiver_balance = receiver.view_account().await?.balance;
        ...
        let confirm_event = check(&contract, &receiver, transfer_id).await?;
